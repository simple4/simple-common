...
import net.simpleframework.lib.org.jsoup.safety.Cleaner;
import net.simpleframework.lib.org.jsoup.safety.Whitelist;
...
public class Jsoup {
	 ...
	 * 
	 * @param url
	 *           URL to fetch (with a GET). The protocol must be {@code http} or
	 *           {@code https}.
	 * @param timeoutMillis
	...
	public static String clean(final String bodyHtml, final String baseUri,
			final Whitelist whitelist, final Document.OutputSettings outputSettings) {
		final Document dirty = parseBodyFragment(bodyHtml, baseUri);
		final Cleaner cleaner = new Cleaner(whitelist);
		final Document clean = cleaner.clean(dirty);
