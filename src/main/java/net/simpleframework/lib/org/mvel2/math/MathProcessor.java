 ...
 */
package net.simpleframework.lib.org.mvel2.math;

import static java.lang.String.valueOf;
import static net.simpleframework.lib.org.mvel2.DataConversion.convert;
import static net.simpleframework.lib.org.mvel2.DataTypes.BIG_DECIMAL;
import static net.simpleframework.lib.org.mvel2.DataTypes.EMPTY;
...
import static net.simpleframework.lib.org.mvel2.Operator.BW_NOT;
...
import static net.simpleframework.lib.org.mvel2.util.ParseTools.isNumber;
import static net.simpleframework.lib.org.mvel2.util.ParseTools.narrowType;
...
public strictfp class MathProcessor {
	...
	public static Object doOperations(final Object val1, final int operation, final Object val2) {
		return doOperations(val1 == null ? DataTypes.OBJECT : __resolveType(val1.getClass()), val1,
				operation, val2 == null ? DataTypes.NULL : __resolveType(val2.getClass()), val2);
	}

	public static Object doOperations(final Object val1, final int operation, final int type2,
			final Object val2) {
		return doOperations(val1 == null ? DataTypes.OBJECT : __resolveType(val1.getClass()), val1,
				operation, type2, val2);
	}

	public static Object doOperations(int type1, final Object val1, final int operation, int type2,
			final Object val2) {
		if (type1 == -1) {
		...
		case BIG_DECIMAL:
			...
			default:
				if (type2 > 99) {
					return doBigDecimalArithmetic((BigDecimal) val1, operation,
							getInternalNumberFromType(val2, type2), false, -1);
				} else {
	...
	private static Object doPrimWrapperArithmetic(final Number val1, final int operation,
			final Number val2, final boolean iNumber, final int returnTarget) {
		switch (operation) {
		case ADD:
	...
	private static Object toType(final Number val, final int returnType) {
		switch (returnType) {
	...
	private static Object doBigDecimalArithmetic(final BigDecimal val1, final int operation,
			final BigDecimal val2, final boolean iNumber, final int returnTarget) {
		switch (operation) {
		case ADD:
			if (iNumber) {
	...
	private static Object _doOperations(final int type1, final Object val1, final int operation,
		...
		if (operation < 20) {
			if (((type1 > 49 || operation == EQUAL || operation == NEQUAL) && type1 == type2)
				...
				return doOperationNonNumeric(type1, convert(val1, Boolean.class), operation,
						convert(val2, Boolean.class));
			}
			// Fix for: MVEL-56
			else if ((type1 == 1 || type2 == 1)
					&& (type1 == 8 || type1 == 112 || type2 == 8 || type2 == 112)) {
				if (type1 == 1) {
	...
	private static boolean isIntegerType(final int type) {
		return type == DataTypes.INTEGER || type == DataTypes.W_INTEGER || type == DataTypes.LONG
				|| type == DataTypes.W_LONG;
	}

	private static Object doOperationNonNumeric(final int type1, final Object val1,
			final int operation, final Object val2) {
		switch (operation) {
		case ADD:
			if (type1 == DataTypes.COLLECTION) {
				final List list = new ArrayList((Collection) val1);
				list.add(val2);
				return list;
			} else {
	...
	private static Object doOperationsSameType(final int type1, Object val1, final int operation,
			Object val2) {
		switch (type1) {
		...
		case DataTypes.W_INTEGER:
			...
			case MULT:
				return ((Integer) val1) * ((Integer) val2);
			case POWER:
				final double d = Math.pow((Integer) val1, (Integer) val2);
			...
			case LETHAN:
				return ((Integer) val1) <= ((Integer) val2) ? Boolean.TRUE : Boolean.FALSE;
			case EQUAL:
				return ((Integer) val1).intValue() == ((Integer) val2).intValue() ? Boolean.TRUE
						: Boolean.FALSE;
			case NEQUAL:
				return ((Integer) val1).intValue() != ((Integer) val2).intValue() ? Boolean.TRUE
						: Boolean.FALSE;
		...
		case DataTypes.W_SHORT:
			...
			case MULT:
				return ((Short) val1) * ((Short) val2);
			case POWER:
				final double d = Math.pow((Short) val1, (Short) val2);
			...
			case LETHAN:
				return ((Short) val1) <= ((Short) val2) ? Boolean.TRUE : Boolean.FALSE;
			case EQUAL:
				return ((Short) val1).shortValue() == ((Short) val2).shortValue() ? Boolean.TRUE
						: Boolean.FALSE;
			case NEQUAL:
				return ((Short) val1).shortValue() != ((Short) val2).shortValue() ? Boolean.TRUE
						: Boolean.FALSE;
		...
		case DataTypes.W_LONG:
			...
			case MULT:
				return ((Long) val1) * ((Long) val2);
			case POWER:
				final double d = Math.pow((Long) val1, (Long) val2);
				if (d > Long.MAX_VALUE) {
					return d;
				} else {
					return (long) d;
			...
			case LETHAN:
				return ((Long) val1) <= ((Long) val2) ? Boolean.TRUE : Boolean.FALSE;
			case EQUAL:
				return ((Long) val1).longValue() == ((Long) val2).longValue() ? Boolean.TRUE
						: Boolean.FALSE;
			case NEQUAL:
				return ((Long) val1).longValue() != ((Long) val2).longValue() ? Boolean.TRUE
						: Boolean.FALSE;
		...
		case DataTypes.W_DOUBLE:
			...
			case LETHAN:
				return ((Double) val1) <= ((Double) val2) ? Boolean.TRUE : Boolean.FALSE;
			case EQUAL:
				return ((Double) val1).doubleValue() == ((Double) val2).doubleValue() ? Boolean.TRUE
						: Boolean.FALSE;
			case NEQUAL:
				return ((Double) val1).doubleValue() != ((Double) val2).doubleValue() ? Boolean.TRUE
						: Boolean.FALSE;
		...
		case DataTypes.W_FLOAT:
			...
			case LETHAN:
				return ((Float) val1) <= ((Float) val2) ? Boolean.TRUE : Boolean.FALSE;
			case EQUAL:
				return ((Float) val1).floatValue() == ((Float) val2).floatValue() ? Boolean.TRUE
						: Boolean.FALSE;
			case NEQUAL:
				return ((Float) val1).floatValue() != ((Float) val2).floatValue() ? Boolean.TRUE
						: Boolean.FALSE;
		...
		case DataTypes.BIG_INTEGER:
			...
			case GTHAN:
				return ((BigInteger) val1).compareTo(((BigInteger) val2)) == 1 ? Boolean.TRUE
						: Boolean.FALSE;
			case GETHAN:
				return ((BigInteger) val1).compareTo(((BigInteger) val2)) >= 0 ? Boolean.TRUE
						: Boolean.FALSE;
			case LTHAN:
				return ((BigInteger) val1).compareTo(((BigInteger) val2)) == -1 ? Boolean.TRUE
						: Boolean.FALSE;
	...
	private static int box(final int type) {
		switch (type) {
		case DataTypes.INTEGER:
