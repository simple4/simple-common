...
import static net.simpleframework.lib.org.mvel2.ast.ASTNode.OPT_SUBTR;
import static net.simpleframework.lib.org.mvel2.util.CompilerTools.finalizePayload;
import static net.simpleframework.lib.org.mvel2.util.CompilerTools.signNumber;
import static net.simpleframework.lib.org.mvel2.util.ParseTools.subCompileExpression;
import static net.simpleframework.lib.org.mvel2.util.ParseTools.unboxPrimitive;
...
import net.simpleframework.lib.org.mvel2.ast.Assignment;
...
public class ExpressionCompiler extends AbstractParser {
				...
				while (iter.hasNext()) {
					...
					if (e.getExpr() != expr) {
						...
						err.append("\n - ").append("(").append(e.getLineNumber()).append(",")
								.append(e.getColumn()).append(")").append(" ").append(e.getMessage());
					}
				}
	...
	public CompiledExpression _compile() {
		...
		final ASTLinkedList astBuild = new ASTLinkedList();
		...
		try {
			...
			while ((tk = nextToken()) != null) {
				...
				if (!verifyOnly && tk.isLiteral()) {
					...
					if ((tkOp = nextTokenSkipSymbols()) != null && tkOp.isOperator()
							&& !tkOp.isOperator(Operator.TERNARY)
						...
						if ((tkLA = nextTokenSkipSymbols()) != null
							...
							/**
							 * Now we need to check to see if this is a continuing
							 * reduction.
							 */
					...
					} else if (tkOp != null && !tkOp.isOperator()
							&& !(tk.getLiteralValue() instanceof Class)) {
						throw new CompileException("unexpected token: " + tkOp.getName(), expr,
								tkOp.getStart());
					} else {
						literalOnly = 0;
			...
			if (!stk.isEmpty()) {
				throw new CompileException("COMPILE ERROR: non-empty stack after compileShared.", expr,
						cursor);
			}
	...
	private boolean compileReduce(final int opCode, final ASTLinkedList astBuild) {
		...
		case -2:
			...
			final OperatorNode operator = new OperatorNode((Integer) stk.pop(), expr, st, pCtx);
	...
	private static boolean isBooleanOperator(final int operator) {
		return operator == Operator.AND || operator == Operator.OR || operator == Operator.TERNARY
				|| operator == Operator.TERNARY_ELSE;
	}

	protected ASTNode verify(final ParserContext pCtx, final ASTNode tk) {
		if (tk.isOperator()
				&& (tk.getOperator().equals(Operator.AND) || tk.getOperator().equals(Operator.OR))) {
		...
		if (tk.isDiscard() || tk.isOperator()) {
			...
			/**
			 * Convert literal values from the default ASTNode to the
			 * more-efficient LiteralNode.
			 */
		...
		if (verifying) {
			if (tk.isIdentifier()) {
				final PropertyVerifier propVerifier = new PropertyVerifier(expr, tk.getStart(),
						tk.getOffset(), pCtx);
				...
				if (tk instanceof Union) {
					...
					if (!propVerifier.isMethodCall() && !returnType.isEnum()
							&& !pCtx.isOptimizerNotified() && pCtx.isStrongTyping()
							&& !pCtx.isVariableVisible(tk.getAbsoluteName()) && !tk.isFQCN()) {
						throw new CompileException("no such identifier: " + tk.getAbsoluteName(), expr,
								tk.getStart());
					}
				}
			} else if (tk.isAssignment()) {
				final Assignment a = (Assignment) tk;

				if (a.getAssignmentVar() != null) {
					// pCtx.makeVisible(a.getAssignmentVar());

					final PropertyVerifier propVerifier = new PropertyVerifier(a.getAssignmentVar(),
							pCtx);
					tk.setEgressType(returnType = propVerifier.analyze());
					...
					final ExecutableStatement c = (ExecutableStatement) subCompileExpression(expr,
					...
					if (pCtx.isStrictTypeEnforcement()) {
						...
						if (!returnType.isAssignableFrom(c.getKnownEgressType()) && c.isLiteralOnly()) {
							if (canConvert(c.getKnownEgressType(), returnType)) {
								...
								/**
								 * We ignore boxed primitive cases, since MVEL does not
		...
		if (!tk.isLiteral() && tk.getClass() == ASTNode.class
				&& (tk.getFields() & ASTNode.ARRAY_TYPE_LITERAL) == 0) {
	...
	public ExpressionCompiler(final String expression, final boolean verifying) {
		setExpression(expression);
		this.verifying = verifying;
	}

	public ExpressionCompiler(final char[] expression) {
		setExpression(expression);
	}
	...
	public ExpressionCompiler(final String expression, final int start, final int offset,
	...
	public void setVerifying(final boolean verifying) {
		this.verifying = verifying;
	}
