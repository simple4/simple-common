...
import static net.simpleframework.lib.org.mvel2.util.ParseTools.findAbsoluteLast;
...
public class PropertyAccessor {
	...
	static {
		READ_PROPERTY_RESOLVER_CACHE = (new WeakHashMap<Class, WeakHashMap<Integer, WeakReference<Member>>>(
				10));
		WRITE_PROPERTY_RESOLVER_CACHE = (new WeakHashMap<Class, WeakHashMap<Integer, WeakReference<Member>>>(
				10));
		METHOD_RESOLVER_CACHE = (new WeakHashMap<Class, WeakHashMap<Integer, WeakReference<Object[]>>>(
				10));
		METHOD_PARMTYPES_CACHE = new WeakHashMap<Member, WeakReference<Class[]>>(10);
	}

	public PropertyAccessor(final String property, final Object ctx) {
		this.length = end = (this.property = property.toCharArray()).length;
		this.ctx = ctx;
		this.variableFactory = new ImmutableDefaultFactory();
	}

	public PropertyAccessor(final char[] property, final Object ctx,
			final VariableResolverFactory resolver, final Object thisReference,
	...
	public PropertyAccessor(final char[] property, final int start, final int offset,
			final Object ctx, final VariableResolverFactory resolver, final Object thisReference,
			final ParserContext pCtx) {
	...
	public static Object get(final String property, final Object ctx) {
		return new PropertyAccessor(property, ctx).get();
	}

	public static Object get(final char[] property, final int offset, final int end,
			final Object ctx, final VariableResolverFactory resolver, final Object thisReferece,
			final ParserContext pCtx) {
	...
	public static void set(final Object ctx, final String property, final Object value) {
		new PropertyAccessor(property, ctx).set(value);
	}

	public static void set(final Object ctx, final VariableResolverFactory resolver,
			final String property, final Object value, final ParserContext pCtx) {
	...
	private Object get() {
		...
		try {
			...
			throw new PropertyAccessException("null pointer exception in property: "
					+ new String(property), property, cursor, e);
		} catch (final Exception e) {
			throw new PropertyAccessException("unknown exception in expression: "
					+ new String(property), property, cursor, e);
		}
	}
	...
	private void set(final Object value) {
		...
		try {
			final int oLength = end;

			end = findAbsoluteLast(property);

			if ((curr = get()) == null) {
				throw new PropertyAccessException("cannot bind to null context: "
						+ new String(property, cursor, length), property, cursor);
			}
			...
			if (nextToken() == COL) {
				...
				final String ex = new String(property, _start, cursor - _start);
				...
				if (!MVEL.COMPILER_OPT_ALLOW_OVERRIDE_ALL_PROPHANDLING) {
					...
					if (curr instanceof Map) {
						...
						if (hasPropertyHandler(List.class)) {
							...
							((List) curr).set(eval(ex, this.ctx, this.variableFactory, Integer.class),
									value);
						}
					} else if (curr.getClass().isArray()) {
					...
					&& hasPropertyHandler(curr.getClass())) {
				getPropertyHandler(curr.getClass())
						.setProperty(capture(), curr, variableFactory, value);
				return;
			}

			final String tk = capture();

			Member member = checkWriteCache(curr.getClass(), tk == null ? 0 : tk.hashCode());
			if (member == null) {
				addWriteCache(
						curr.getClass(),
						tk != null ? tk.hashCode() : -1,
			...
			if (member instanceof Method) {
				...
				final Class[] paramaterTypes = checkParmTypesCache(meth);
				...
				if (value != null && !paramaterTypes[0].isAssignableFrom(value.getClass())) {
					if (!canConvert(paramaterTypes[0], value.getClass())) {
						throw new CompileException("cannot convert type: " + value.getClass() + ": to "
								+ meth.getParameterTypes()[0], property, cursor);
					}
	...
	public static void reportCacheSizes() {
		...
		for (final Class cls : READ_PROPERTY_RESOLVER_CACHE.keySet()) {
			System.out.println(" [" + cls.getName() + "]: "
					+ READ_PROPERTY_RESOLVER_CACHE.get(cls).size() + " entries.");
		}
		System.out.println("write property cache: " + WRITE_PROPERTY_RESOLVER_CACHE.size());
		for (final Class cls : WRITE_PROPERTY_RESOLVER_CACHE.keySet()) {
			System.out.println(" [" + cls.getName() + "]: "
					+ WRITE_PROPERTY_RESOLVER_CACHE.get(cls).size() + " entries.");
		}
		System.out.println("method cache: " + METHOD_RESOLVER_CACHE.size());
		for (final Class cls : METHOD_RESOLVER_CACHE.keySet()) {
			System.out.println(" [" + cls.getName() + "]: " + METHOD_RESOLVER_CACHE.get(cls).size()
					+ " entries.");
		}
	}

	private static void addReadCache(final Class cls, final Integer property, final Member member) {
		synchronized (READ_PROPERTY_RESOLVER_CACHE) {
			WeakHashMap<Integer, WeakReference<Member>> nestedMap = READ_PROPERTY_RESOLVER_CACHE
					.get(cls);
	...
	private static Member checkReadCache(final Class cls, final Integer property) {
		final WeakHashMap<Integer, WeakReference<Member>> map = READ_PROPERTY_RESOLVER_CACHE.get(cls);
		if (map != null) {
			final WeakReference<Member> member = map.get(property);
			if (member != null) {
				return member.get();
			}
		}
	...
	private static void addWriteCache(final Class cls, final Integer property, final Member member) {
	...
	private static Member checkWriteCache(final Class cls, final Integer property) {
		final Map<Integer, WeakReference<Member>> map = WRITE_PROPERTY_RESOLVER_CACHE.get(cls);
		if (map != null) {
			final WeakReference<Member> member = map.get(property);
			if (member != null) {
				return member.get();
	...
	public static Class[] checkParmTypesCache(final Method member) {
	...
	private static void addMethodCache(final Class cls, final Integer property, final Method member) {
	...
	private static Object[] checkMethodCache(final Class cls, final Integer property) {
		final Map<Integer, WeakReference<Object[]>> map = METHOD_RESOLVER_CACHE.get(cls);
		if (map != null) {
			final WeakReference<Object[]> ref = map.get(property);
			if (ref != null) {
				return ref.get();
			}
		}
	...
	private Object getBeanProperty(final Object ctx, final String property)
		...
		if (ctx != null) {
			...
			if (member instanceof Method) {
					...
					if (member.getDeclaringClass().equals(ctx)) {
						try {
							...
							throw new CompileException("name collision between innerclass: "
									...
									+ member.toString() + ")", this.property, this.st);
				...
				if (ctx instanceof Proto.ProtoInstance) {
					return ((Proto.ProtoInstance) ctx).get(property).call(null, thisReference,
							variableFactory, EMPTY_OBJ_ARR);
				}
		...
		final Object tryStatic = tryStaticAccess();
		...
		if (ctx == null) {
			...
			throw new PropertyAccessException("could not access: " + property + "; in class: "
					+ ctx.getClass().getName(), this.property, st);
		}
	}
	 ...
	 * @return - returns true is end of statement is hit, false if the scan scar
	 *         is countered.
	 */
	private boolean scanTo(final char c) {
	...
	private Object getWithProperty(final Object ctx) {
		...
		final String nestParm = start == cursor ? null : new String(property, start, cursor - start
				- 1).trim();
	...
	private Object getCollectionProperty(Object ctx, String prop) throws Exception {
		...
		if (ctx instanceof Map) {
			...
			try {
				return getClassReference(getCurrentThreadParserContext(), (Class) ctx,
						new TypeDescriptor(property, start, length, 0));
			} catch (final Exception e) {
				throw new PropertyAccessException("illegal use of []: unknown type: "
						+ (ctx.getClass().getName()), property, st, e);
			}
		}
	}
	...
	private Object getCollectionPropertyAO(Object ctx, String prop) throws Exception {
		...
		if (ctx instanceof Map) {
			...
			if (hasPropertyHandler(Collection.class)) {
				...
				final int count = (Integer) eval(prop, ctx, variableFactory);
				if (count > ((Collection) ctx).size()) {
					throw new PropertyAccessException("index [" + count
							+ "] out of bounds on collections", property, cursor);
				}
			...
			try {
				return getClassReference(getCurrentThreadParserContext(), (Class) ctx,
						new TypeDescriptor(property, start, end - start, 0));
			} catch (final Exception e) {
				throw new PropertyAccessException("illegal use of []: unknown type: "
						+ (ctx.getClass().getName()), property, st);
			}
		}
	}
	...
	private Object getMethod(Object ctx, String name) {
		...
		final String tk = cursor != end && property[cursor] == '('
				&& ((cursor = balancedCapture(property, cursor, '(')) - _start) > 1 ? new String(
				property, _start + 1, cursor - _start - 1) : "";
		...
		if (cls == Proto.ProtoInstance.class) {
			return ((Proto.ProtoInstance) ctx).get(name).call(null, thisReference, variableFactory,
					args);
		}
		 ...
		 */
		final Object[] cache = checkMethodCache(cls, createSignature(name, tk));
		...
		if (m == null) {
			...
			try {
				...
				try {
					...
					throw new PropertyAccessException("unable to invoke method: " + name, property,
							cursor, e2);
				}
			} catch (final RuntimeException e) {
				throw e;
			} catch (final Exception e) {
				throw new PropertyAccessException("unable to invoke method: " + name, property, cursor,
						e);
			}
		}
	}

	private static int createSignature(final String name, final String args) {
				...
				case '.':
					if (!meth) {
						try {
							...
							final Class cls = getClassLoader().loadClass(
									new String(property, start, i - start));
							final String name = new String(property, i + 1, end - i - 1);
							try {
