...
enum HtmlTreeBuilderState {
	Initial {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			if (isWhitespace(t)) {
				...
				final DocumentType doctype = new DocumentType(d.getName(), d.getPublicIdentifier(),
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			if (t.isDoctype()) {
				...
				tb.transition(BeforeHead);
					...
					&& (StringUtil.in(t.asEndTag().name(), "head", "body", "html", "br"))) {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			...
			case StartTag:
				...
				if (name.equals("html")) {
					...
					final Element meta = tb.insertEmpty(start);
	...
	InBody {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			...
			case StartTag:
				...
				if (name.equals("html")) {
					...
					final Element html = tb.getStack().getFirst();
					...
					tb.error(this);
					final LinkedList<Element> stack = tb.getStack();
					if (stack.size() == 1
				...
				} else if (StringUtil.in(name, "address", "article", "aside", "blockquote", "center",
					...
					if (StringUtil
							.in(tb.currentElement().nodeName(), "h1", "h2", "h3", "h4", "h5", "h6")) {
						tb.error(this);
						tb.pop();
					}
					tb.insert(startTag);
					...
					final Element form = tb.insert(startTag);
				...
				} else if (StringUtil.in(name, "b", "big", "code", "em", "font", "i", "s", "small",
						"strike", "strong", "tt", "u")) {
					tb.reconstructFormattingElements();
					final Element el = tb.insert(startTag);
					...
					tb.reconstructFormattingElements();
					final Element el = tb.insertEmpty(startTag);
					...
					tb.insert(startTag);
					tb.tokeniser.acknowledgeSelfClosingFlag();
				} else if (StringUtil.in(name, "caption", "col", "colgroup", "frame", "head", "tbody",
			...
			case EndTag:
				...
				if (name.equals("body")) {
					if (!tb.inScope("body")) {
						...
						// todo: error if stack contains something not dd, dt, li,
						// optgroup, option, p, rp, rt, tbody, td, tfoot, th, thead,
						// tr, body, html
						tb.transition(AfterBody);
					}
				} else if (name.equals("html")) {
					final boolean notIgnored = tb.process(new Token.EndTag("body"));
					if (notIgnored) {
						return tb.process(endTag);
					}
				} else if (StringUtil.in(name, "address", "article", "aside", "blockquote", "button",
						"center", "details", "dir", "div", "dl", "fieldset", "figcaption", "figure",
						"footer", "header", "hgroup", "listing", "menu", "nav", "ol", "pre", "section",
						"summary", "ul")) {
					...
					if (currentForm == null || !tb.inScope(name)) {
						...
						// remove currentForm from stack. will shift anything under
						// up.
				...
				} else if (StringUtil.in(name, "a", "b", "big", "code", "em", "font", "i", "nobr", "s",
					...
					OUTER: for (int i = 0; i < 8; i++) {
						...
						// todo: Let a bookmark note the position of the formatting
						// element in the list of active formatting elements relative
						...
						INNER: for (int j = 0; j < 3; j++) {
							...
							final Element replacement = new Element(Tag.valueOf(node.nodeName()),
									tb.getBaseUri());
							...
							if (lastNode == furthestBlock) {
								...
								// immediately after the new node in the list of active
								// formatting elements.
								// not getting how this bookmark both straddles the
								// element above, but is inbetween here...
							}
						...
						final Element adopter = new Element(Tag.valueOf(name), tb.getBaseUri());
				...
				break;
			case EOF:
				// todo: error if stack contains something not dd, dt, li, p, tbody,
		...
		boolean anyOtherEndTag(final Token t, final HtmlTreeBuilder tb) {
			...
			final DescendableLinkedList<Element> stack = tb.getStack();
	...
	Text {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			if (t.isCharacter()) {
				...
				// if: An end tag whose tag name is "script" -- scripting nesting
				// level, if evaluating scripts
				tb.pop();
				tb.transition(tb.originalState());
			}
			return true;
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			if (t.isCharacter()) {
				...
				if (name.equals("caption")) {
					...
					tb.error(this);
					final boolean processed = tb.process(new Token.EndTag("table"));
					if (processed) {
						return tb.process(t);
					}
					...
					if (tb.getFormElement() != null) {
						...
						final Element form = tb.insertEmpty(startTag);
						tb.setFormElement(form);
					}
				} else {
					return anythingElse(t, tb);
				...
				} else if (StringUtil.in(name, "body", "caption", "col", "colgroup", "html", "tbody",
						"td", "tfoot", "th", "thead", "tr")) {
					tb.error(this);
					return false;
				} else {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			...
			default:
				if (tb.getPendingTableCharacters().size() > 0) {
					for (final Token.Character character : tb.getPendingTableCharacters()) {
						if (!isWhitespace(character)) {
							// InTable anything else section:
							tb.error(this);
							if (StringUtil.in(tb.currentElement().nodeName(), "table", "tbody", "tfoot",
									"thead", "tr")) {
								tb.setFosterInserts(true);
	...
	InCaption {
		...
		boolean process(final Token t, final HtmlTreeBuilder tb) {
			if (t.isEndTag() && t.asEndTag().name().equals("caption")) {
				...
				tb.error(this);
				final boolean processed = tb.process(new Token.EndTag("caption"));
				if (processed) {
					return tb.process(t);
				}
			} else if (t.isEndTag()
	...
	InColumnGroup {
		...
		private boolean anythingElse(final Token t, final TreeBuilder tb) {
			final boolean processed = tb.process(new Token.EndTag("colgroup"));
			if (processed) {
				return tb.process(t);
			}
			return true;
	...
	InRow {
		...
		private boolean handleMissingTr(final Token t, final TreeBuilder tb) {
			final boolean processed = tb.process(new Token.EndTag("tr"));
			if (processed) {
				return tb.process(t);
			} else {
				return false;
